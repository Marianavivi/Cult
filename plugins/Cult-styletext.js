...
async function handler(m, { args }) {
    try {
        ...
        if (!args[0]) {
            const styleList = styles.map((style, index) => `${index + 1}. ${style.styleName || 'Style ' + (index + 1)}`).join('\n');
        ...
        await m.reply('An error occurred while styling the text. Please try again later.');
    }
}
